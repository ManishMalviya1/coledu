...
<?php
   ...
   $sql = mysqli_query($conn,"SELECT distinct title,link,takes.cid as cid,source,duration from  courses inner join takes inner join registration on registration.id=takes.id and takes.cid=courses.cid where email='{$username}'||contact='{$username}' order by title");
